...
import { Brain, ArrowRight, Lock, Mail, User, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetchApi<{ user: UserProfile & { token: string } }>('/auth/register', {
        method: 'POST',
      ...
      if (res.success && res.data?.user?.token) {
        ...
        setErrorMsg(res.message || 'Registration failed. Please try a different email address.');
      }
    } catch (err: any) {
      setIsLoading(false);
  ...
  return (
    <div className="min-h-screen bg-obsidian-bg text-obsidian-text flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute -top-40 -right-40 w-96 h-96 bg-obsidian-purple/15 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md bg-obsidian-sidebar border border-obsidian-border p-8 rounded-2xl shadow-2xl z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-lg font-semibold text-white pt-2">Create Account</h1>
          <p className="text-xs text-obsidian-muted">Join ExamPrep AI to start practicing adaptive exam quizzes</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-obsidian-muted uppercase tracking-wider">Full Name</label>
            <div className="relative">
              <User className="absolute left-3 top-2.5 text-obsidian-muted" size={16} />
              <input
                type="text"
                placeholder="Aspirant Student"
                value={name}
        ...
        <div className="text-center text-xs text-obsidian-muted pt-2">
          ...
          <Link href="/login" className="text-obsidian-purple font-semibold hover:underline">
            Sign In
          </Link>
        </div>
      </div>
